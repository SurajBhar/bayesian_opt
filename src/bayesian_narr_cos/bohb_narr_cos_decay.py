...
def create_fine_tuning_config_space(cfg: DictConfig):
    ...
    try:
        ...
        T_max = CS.Constant("T_max", cfg.hyperparameters.scheduler.T_max),
    ...
    def _initialize_scheduler(self, config):
        ...
        try:
            ...
            if scheduler_type == "CosineAnnealingLR":
                ...
                # Initialize CosineAnnealingLR
                T_max = config["T_max"]
                eta_min = config["eta_min"]  # Provide a default value if not specified
...
try:
    from config_narr_cos import register_configs  # Importing custom config registration
    register_configs()
except ImportError as e:
...
@hydra.main(
    version_base=None, config_path="../../conf", config_name="config_narr_cos_decay"
)  # Hydra Decorator
